    ...
    <div className="min-h-screen bg-[#0d1117] text-[#c9d1d9] font-sans flex flex-col items-center justify-center px-4 py-12 selection:bg-[#58a6ff]/30 selection:text-white relative">
      ...
      <div className="relative z-10 w-full max-w-[340px] flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <h1 className="text-2xl font-light text-white tracking-tight">Create your G-Remover account</h1>
        </div>
        ...
        <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-5 flex flex-col gap-4 shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="bg-[#0d1117] border border-[#30363d] rounded-md p-3 flex flex-col gap-2 mt-1 text-xs select-none">
                <span className="font-semibold text-white/95">Password Requirements:</span>
                <ul className="flex flex-col gap-1.5 text-[#8b949e]">
                  <li className={`flex items-center gap-2 ${password.trim().length >= 8 ? "text-[#3fb950]" : ""}`}>
                    <span>{password.trim().length >= 8 ? "✓" : "○"}</span>
                    <span>Minimum 8 characters</span>
                  </li>
                  <li className={`flex items-center gap-2 ${/[A-Z]/.test(password.trim()) ? "text-[#3fb950]" : ""}`}>
                    <span>{/[A-Z]/.test(password.trim()) ? "✓" : "○"}</span>
                    <span>At least one uppercase letter (A-Z)</span>
                  </li>
                  <li className={`flex items-center gap-2 ${/[0-9]/.test(password.trim()) ? "text-[#3fb950]" : ""}`}>
                    <span>{/[0-9]/.test(password.trim()) ? "✓" : "○"}</span>
                    <span>At least one number (0-9)</span>
                  </li>
                  <li className={`flex items-center gap-2 ${/[^a-zA-Z0-9]/.test(password.trim()) ? "text-[#3fb950]" : ""}`}>
                    <span>{/[^a-zA-Z0-9]/.test(password.trim()) ? "✓" : "○"}</span>
        ...
        <div className="border border-[#30363d] rounded-lg p-4 text-center text-xs">
          ...
          <Link href="/auth/login" className="text-[#58a6ff] hover:underline font-semibold">
            Sign in
          </Link>
          <span>.</span>
        </div>
